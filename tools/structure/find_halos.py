...
def group_sizes(path: pathlib.Path) -> tuple[np.ndarray, float]:
    """(particles per FOF group, box size in Mpc) for one snapshot."""
    ...
    if len(pairs) == 0:
        ...
        return np.ones(n, dtype=np.int64), box
...
def run_one(run_dir: pathlib.Path, omega_m: float) -> dict:
    ...
    counts, biggest, n_particles = [], [], None
    for i, snap in enumerate(snaps):
        t0 = time.time()
        sizes, _ = group_sizes(snap)
